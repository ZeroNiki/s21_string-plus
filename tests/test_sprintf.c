...
START_TEST(sprintf_precision_no_dot) {
  char buffer[100];
  ck_assert_int_eq(s21_sprintf(buffer, "abc"), sprintf(buffer, "abc"));
}
END_TEST

START_TEST(precision_dot_no_minus) {
  char buffer[100];
  ck_assert_int_eq(s21_sprintf(buffer, "%.3d", 42),
                   sprintf(buffer, "%.3d", 42));
}
END_TEST
...
Suite *create_sprintf_test(void) {
  ...
  tcase_add_test(tc, sprintf_precision_no_dot);
  tcase_add_test(tc, precision_dot_no_minus);
